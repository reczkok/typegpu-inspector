...
describe('statement-map diagnostics', () => {
  ...
  it('folds a caller\'s compiler message into the helper\'s own report of the statement', async () => {
    ...
    expect(helper!.relatedInformation?.map((info) => info.message)).toEqual(['in fn stepBoid']);
    expect(helper!.data).toMatchObject({ affectedTargets: ['mainCompute'] });
  });
...
describe('cross-file statement-map diagnostics', () => {
  ...
  it('hovers a cross-file diagnostic with a link to the statement', async () => {
    ...
    expect(markdown).toContain('Also affects 1 target: `mainCompute`.');
  ...
  it('anchors on the target when it does not call the imported helper itself', async () => {
    ...
    expect(diagnostic!.relatedInformation?.[0]).toMatchObject({
      ...
      message: 'in rotateXY via stepBoid',
    });
    expect(diagnostic!.message).toContain(
